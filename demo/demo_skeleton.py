...
sys.path.append('/content/drive/MyDrive/Neural Networks Project/pyskl')
...
def parse_args():
    ...
    parser.add_argument(
        '--det-checkpoint',
        default='demo/faster_rcnn_r50_fpn_1x_coco-person_20201216_175929-d022e227.pth',
    ...
    parser.add_argument(
        '--label-map',
        default='tools/data/label_map/nturgbd_5.txt',
        help='label map file')
    parser.add_argument(
...
def main():
    ...
    if GCN_flag:
        ...
        GCN_nperson = format_op.get('num_person', GCN_nperson)
    ...
    pose_results_copy = pose_results.copy()
    ...
    for j in range(num_person):
        ...
        fake_anno['keypoint'] = np.array([list(keypoint[j])])
        fake_anno['keypoint_score'] = np.array([list(keypoint_score[j])])

        fake_anno_list.append(fake_anno)

    action_label_list = []
    for j in range(num_person):
        results = inference_recognizer(model, fake_anno_list[j])
    ...
    vis_frames = [vis_pose_result(pose_model, frame_paths[i], pose_results[i]) for i in
                  range(num_frame)]

    for i, poses in enumerate(pose_results_copy):
        for j, pose in enumerate(poses):

            if len(pose) > 0:

                firstCoordinate = pose.get('bbox', [])[0]
                secondCoordinate = pose.get('bbox', [])[3]

                cv2.putText(vis_frames[i], action_label_list[j], (int(firstCoordinate), int(secondCoordinate)), FONTFACE, FONTSCALE,
